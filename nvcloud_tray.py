...
def criar_icone(status: str = "online") -> Image.Image:
    """
    Gera ícone hexagonal com pulso ECG nas cores da marca.
    status: "online"  → hexágono navy  + pulso verde  (#2DBD8F)
            "offline" → hexágono cinza + pulso branco
            "alerta"  → hexágono navy  + pulso laranja (#F59E0B)
    """
    import math
    ...
    borda_cor = BORDA.get(status, "#2DBD8F")
    pulso_cor = PULSO.get(status, "#2DBD8F")
    fundo_cor = FUNDO.get(status, NAVY)
    ...
    draw.polygon(pts, fill=fundo_cor)
    draw.polygon(pts, outline=NAVY, width=3)
    draw.polygon(pts, outline=borda_cor, width=2)

    # Pulso ECG
    pulse = [(8,32),(16,32),(20,20),(25,44),(29,26),(33,38),(37,32),(56,32)]
    draw.line(pulse, fill=pulso_cor, width=3)
